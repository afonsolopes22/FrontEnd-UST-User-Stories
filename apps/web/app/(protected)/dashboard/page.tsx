                    ...
                    {teams.length > 0 && (
                        <label className={styles.filterSelect} title="Which team's history to show. Scope (team / project / organization) is set by your admin in Settings.">
                            ...
                            <select
                                ...
                                onChange={e => { setTeamFilter(e.target.value === 'all' ? null : Number(e.target.value)); resetPage() }}
                            >
                                <option value="all">All my teams</option>
                                {teams.map(t => (
                                    <option key={t.id} value={t.id}>
                                        {t.name} · {t.history_visibility === 'organization' ? 'org-wide' : t.history_visibility === 'project' ? 'project-wide' : 'team-only'}
                                    </option>
                                ))}
                            </select>
                        </label>
