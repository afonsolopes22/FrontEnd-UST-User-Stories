...
// Fetches every registered best practice for a project (the evaluate endpoint
// currently always returns an empty best_practices_feedback array).
async function fetchProjectBestPractices(orgId: number, projectId: number, token: string | null): Promise<string[]> {
  const descriptions: string[] = []
  let page = 1
  let pages = 1
  do {
    const res = await fetch(`/api/organizations/${orgId}/projects/${projectId}/best-practices?page=${page}&limit=100`, {
      headers: token ? { Authorization: `Bearer ${token}` } : undefined,
    })
    if (!res.ok) break
    const data = await res.json()
    for (const bp of data?.best_practices ?? []) {
      if (bp?.description) descriptions.push(bp.description)
    }
    pages = data?.pages ?? 1
    page += 1
  } while (page <= pages)
  ...
  useEffect(() => {
    ...
    async function runEvaluation() {
      try {
        ...
        if (!data.best_practices_feedback || data.best_practices_feedback.length === 0) {
          const registeredPractices = await fetchProjectBestPractices(storedOrg.id, parseInt(projectId), token).catch(() => [])
          if (registeredPractices.length > 0) data.best_practices_feedback = registeredPractices
        }
